...
open class SkinnableShapeableImageView @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0
) : ShapeableImageView(context, attrs, defStyleAttr), ViewsMatch {
    ...
    override fun setBackgroundResource(resId: Int) {
        attrsBean.updateViewResource(R.styleable.SkinnableShapeableImageView[R.styleable.SkinnableShapeableImageView_android_background], resId)
    ...
    override fun skinnableView() {
        ...
        // background
        var key = R.styleable.SkinnableShapeableImageView[R.styleable.SkinnableShapeableImageView_android_background]
        ...
        // src
        key = R.styleable.SkinnableShapeableImageView[R.styleable.SkinnableShapeableImageView_android_src]
        val srcResourceId = attrsBean.getViewResource(key)
        if (srcResourceId > 0) {
            if (manager.isDefaultSkin) {
                setImageDrawable(ContextCompat.getDrawable(context, srcResourceId))
            } else {
                when (val skinResource = manager.getBackgroundOrSrc(srcResourceId)) {
